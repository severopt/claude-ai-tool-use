...
function generateGoogleMapsUrl(travelPlan: TravelPlan): string {
  ...
  const sortedDestinations = [...travelPlan.destinations]
  ...
  // Construct URL
  const url = `${baseUrl}&origin=${origin}&destination=${destination}${waypoints ? `&waypoints=${waypoints}` : ''}&travelmode=driving`;
